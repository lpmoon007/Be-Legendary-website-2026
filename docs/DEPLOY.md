...
|---|---|---|---|
| **Legacy 301 redirects (nginx)** | ✅ Not needed (insurance) | **Verified 2026-09-11:** `curl -sI https://www.belegendary.org/shakubuku/` returns `301 → /teams/retreats/`, so Plesk IS honoring the `.htaccess` redirects in production — they are NOT bypassed. The nginx port is kept only as insurance in case Plesk's static-serving mode ever changes; no action required now. | `redirects/plesk-nginx-redirects.conf` |
